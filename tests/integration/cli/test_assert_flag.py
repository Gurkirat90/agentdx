"""`agentdx run --assert PATH OP VALUE` (CLI-invented, PRD §44.1's G1 gate — see module
docstrings in `cli/commands/run.py::_parse_assert_expr` and
...
def test_assert_state_conflict_and_race_are_the_same_check(
    ...
    """`findings.race` and `findings.state_conflict` must count identically (alias, not a second check)."""
...
def test_assert_unsupported_path_is_a_usage_error(
    ...
    """Only `findings.<type>` paths are supported — anything else is a clear usage error, not
    a silent no-op or a fabricated pass."""
    result = cli_runner.invoke(
